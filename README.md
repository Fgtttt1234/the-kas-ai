...
**الاسم بالعربي:** كازاريلا ما بي تي  
**الاسم بالإنجليزي:** Kasarella AI MBT

**المؤسس:** المهندس الخبير الدغريري  
**تاريخ التأسيس:** 25-01-2026

**الوصف:** منصة ذكاء اصطناعي تقنية احترافية للتفكير، التحليل، وصناعة القرار.
...
### استدعاء الهوية برمجياً - Programmatic Identity Access

يمكنك استخدام نظام الهوية الرسمية في الكود الخاص بك:

```python
from core.identity import banner, identity

# عرض البانر الرسمي - Display official banner
print(banner())

# الحصول على معلومات الهوية - Get identity information
info = identity()
print(f"الاسم: {info['name_ar']}")
print(f"المؤسس: {info['founder']}")
print(f"التأسيس: {info['founded']}")
```

أو يمكنك استيراد الحقول مباشرة:

```python
from core.identity import name_ar, name_en, founder, founded

print(f"{name_ar} ({name_en})")
print(f"Founded by {founder} on {founded}")
```
